...
const ServiceCard = memo(({ name, icon: Icon, path }) => {
  const CardContent = (
    ...
    <>
      <header className="md:hidden fixed top-0 w-full px-4 py-2.5 flex justify-center items-center bg-white/95 backdrop-blur-sm z-50 shadow-sm border-b border-gray-100">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="text-[17px] font-bold tracking-tight text-graphite">Legal<span className="text-electric">Click</span></span>
        </Link>
      </header>
...
const MobileBottomBar = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 px-4 pt-1.5 pb-1.5 flex justify-around items-center z-50 safe-bottom">
      <button onClick={() => handleNavClick('home')} className={`flex flex-col items-center gap-0.5 py-1 min-w-[56px] cursor-pointer transition-colors ${activeTab === 'home' && isHome ? 'text-electric' : 'text-gray-400 hover:text-gray-600'}`}>
        <Home className="w-5 h-5" strokeWidth={2.5} />
        <span className={`text-[10px] ${activeTab === 'home' && isHome ? 'font-bold' : 'font-medium'}`}>Головна</span>
      </button>
      <button onClick={() => handleNavClick('services')} className={`flex flex-col items-center gap-0.5 py-1 min-w-[56px] cursor-pointer transition-colors ${activeTab === 'services' && isHome ? 'text-electric' : 'text-gray-400 hover:text-gray-600'}`}>
        ...
        >
          ...
          <div className="flex gap-4 md:grid md:grid-cols-3 md:gap-6 overflow-x-auto no-scrollbar scroll-touch pb-4 md:pb-0 -mx-4 px-4 md:mx-0 md:px-0 snap-x snap-mandatory">
            ...
            <div className="min-w-[260px] md:min-w-0 md:w-full bg-white rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-10 snap-center shrink-0 shadow-lg md:shadow-xl shadow-blue-900/5 md:transition-all md:duration-500 md:hover:-translate-y-3 md:hover:shadow-[0_20px_40px_-15px_rgba(0,82,255,0.2)] border border-slate-100 relative overflow-hidden">
              ...
              <h3 className="font-bold text-base md:text-xl mb-3 md:mb-4 text-electric relative z-10">⚡️ Оперативний зв'язок та старт роботи</h3>
              <ul className="space-y-2 md:space-y-3 text-gray-500 text-sm leading-relaxed list-disc pl-5 relative z-10">
                ...
                <li>Початок роботи в день звернення</li>
              </ul>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
            {serviceItems.map((item, index) => (
              <ServiceCard key={index} name={item.name} icon={item.icon} path={item.path} />
            ))}
          </div>
        </motion.div>
      </section>
