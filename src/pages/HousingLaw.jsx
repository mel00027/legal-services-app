...
import {
  FileText, Home,
  ShieldCheck, MessageCircle, ScrollText, ArrowRight
} from 'lucide-react';
...
const services = [
  { title: "Квартирний облік", desc: "Оскарження зняття та поновлення в черзі на житло.", icon: FileText, color: "from-blue-500 to-indigo-600" },
  { title: "Компенсація за житло", desc: "Отримання грошових виплат на житлове приміщення військовослужбовцям.", icon: ShieldCheck, color: "from-emerald-500 to-teal-600" },
];

const stats = [
  { value: '150+', label: 'Житлових справ' },
...
export const HousingLaw = () => {
  const { data: cms } = useSanityQuery(SERVICE_PAGE_QUERY, null, { slug: 'housing-law' });
  const { data: siteConfig } = useSanityQuery(SITE_CONFIG_QUERY, null);

  const BOT_LINK = siteConfig?.botLink || DEFAULT_BOT_LINK;
  const hero = cms?.hero || {};
  const badgeText = hero.badge || 'Житлове право';
  const titleStart = hero.titleStart || 'Житлове право та';
  const titleGradient = hero.titleGradient || 'захист нерухомості';
  const heroSubtitle = hero.subtitle || 'Поновлення в черзі на житло, отримання компенсацій та повернення коштів при купівлі нерухомості. Працюємо на результат.';
  ...
  const activeExpertise = cms?.expertise || { heading: 'Захищаємо ваше право на житло', description: 'Допомагаємо у питаннях квартирного обліку, компенсацій за житло, повернення коштів та оскарженні рішень. Швидко, прозоро і без черг.' };
  ...
  const seo = cms?.seo || { title: 'Житлове право та захист нерухомості — LegalClick', description: 'Професійні послуги у сфері житлового права: квартирний облік, компенсації, повернення 1% до ПФУ та списання комунальних боргів.' };
      ...
      <section className="relative overflow-hidden bg-transparent py-16 md:py-28 lg:py-36">
        ...
        >
          ...
          >
            ...
            <span className="bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent"> {titleGradient}</span>
          </motion.h1>
      ...
      <section className="px-5 py-14 md:py-24 bg-transparent">
        ...
        >
          <motion.div variants={fadeUpSoft} style={{ willChange: 'transform, opacity' }}
            className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg shadow-emerald-500/20"
          >
      ...
      <section className="px-4 md:px-5 py-12 md:pb-32 bg-transparent text-center">
        ...
        >
          <motion.div variants={cardRise} style={{ willChange: 'transform, opacity' }}
            className="w-14 h-14 md:w-20 md:h-20 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl md:rounded-3xl flex items-center justify-center mx-auto mb-6 md:mb-8 shadow-[0_8px_32px_rgba(16,185,129,0.35)]"
          >
